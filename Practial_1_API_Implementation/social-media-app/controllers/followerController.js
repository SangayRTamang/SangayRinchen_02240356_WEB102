...
exports.getFollower = asyncHandler(async (req, res, next) => {
  ...
  if (!follower) {
    return next(new ErrorResponse(`Follower record not found with id of ${req.params.id}`, 404));
  }
...
exports.createFollower = asyncHandler(async (req, res, next) => {
  ...
  if (!userToFollow) { return next(new ErrorResponse('User to follow not found', 404)); }
...
// @desc    Update follower record (not typically used)
...
exports.deleteFollower = asyncHandler(async (req, res, next) => {
  ...
  if (follower.follower_id !== userId) {
    return next(new ErrorResponse('Not authorized to remove this follow', 401));
  }
